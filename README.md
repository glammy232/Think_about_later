...
Backend для сервиса совместного учета расходов семьи или группы друзей. Поддерживает PostgreSQL/Supabase; без строки подключения автоматически запускается во временном in-memory режиме.
...
Приложение стартует без групп, участников и операций. На `http://localhost:8000/demo` пользователь создаёт группу и свой профиль, после чего frontend сохраняет полученные `group_id` и `user_id` в `localStorage`. В API-запросах текущий пользователь передаётся заголовком `X-User-Id`.
...
- Финансовый помощник: анализ финансов и создание операций через подтверждаемые черновики;
...
Схема и миграции находятся в `supabase/migrations`. Примените их через Supabase CLI или SQL Editor, затем добавьте строку подключения в `.env`:

```dotenv
DATABASE_URL=postgresql://postgres:password@host:5432/postgres
```

После перезапуска API автоматически использует `PostgresStorage`; без `DATABASE_URL` используется временный `InMemoryStorage`. Текущий режим виден в `GET /api/health`. Пароль и полную строку подключения нельзя коммитить — `.env` исключён из Git.
